...
**记忆**同样刻意简单：单表 + 关键词 LIKE 检索，不做向量。置顶与高热的前 10 条作为"常驻核心记忆"注入 Agent 系统提示。对外三条通道（网关 REST `/v1/memories`、网关 MCP、`omi memory mcp` stdio）共用 `memory-api.ts` 里的同一份定义与 handler，行为一致。
